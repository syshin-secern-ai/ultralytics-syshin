...
NAME=$(TZ=Asia/Seoul date +%y%m%d)_golf_flag
DATA=${DATA:-../../data/golf-flag/data.yaml}
...
# INT8 calibration targets ~300 val images; this valset has only 49, so use all of them.
yolo detect export \
    model=runs/detect/${NAME}/weights/best.pt \
    format=onnx \
    quantize=8 \
